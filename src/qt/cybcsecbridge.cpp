...
#include "cybcsecbridge.moc"

CybCSecBridge::CybCSecBridge(CybCSecGUI *window, QObject *parent) :
    QObject         (parent),
    window          (window),
...
void CybCSecBridge::setClientModel()
{
...
// This is just a hook, we won't really be setting the model...
void CybCSecBridge::setWalletModel()
{
...
// This is just a hook, we won't really be setting the model...
void CybCSecBridge::setMessageModel()
{
    populateMessageTable();
    connectSignals();
}

void CybCSecBridge::copy(QString text)
{
    QApplication::clipboard()->setText(text);
}

void CybCSecBridge::paste()
{
    emitPaste(QApplication::clipboard()->text());
}

// Options
void CybCSecBridge::populateOptions()
{
...
// Transactions
bool CybCSecBridge::addRecipient(QString address, QString label, QString narration, qint64 amount, int txnType, int nRingSize)
{
...
void CybCSecBridge::clearRecipients()
{
    recipients.clear();
}

bool CybCSecBridge::sendCoins(bool fUseCoinControl, QString sChangeAddr)
{
    int inputTypes = -1;
    int nAnonOutputs = 0;
    int ringSizes = -1;
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        switch(rcp.txnTypeInd)
        {
            ...
            case TXT_XCS_TO_ANON:
                formatted.append(tr("<b>%1</b> to CYBCSEC %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::XCS, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_ANON:
                formatted.append(tr("<b>%1</b> CYBCSEC, ring size %2 to CYBCSEC %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::XCS, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_XCS:
                formatted.append(tr("<b>%1</b> CYBCSEC, ring size %2 to XCS %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::XCS, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                break;
            default:
    ...
    switch(sendstatus.status)
    {
        ...
        case WalletModel::SCR_StealthAddressFail:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        case WalletModel::SCR_AmountWithFeeExceedsCybCSecBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                tr("The total exceeds your CybCSec balance when the %1 transaction fee is included.").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::XCS, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
...
void CybCSecBridge::openCoinControl()
{
...
void CybCSecBridge::updateCoinControlAmount(qint64 amount)
{
    CoinControlDialog::payAmounts.clear();
...
void CybCSecBridge::updateCoinControlLabels(unsigned int &quantity, int64_t &amount, int64_t &fee, int64_t &afterfee, unsigned int &bytes, QString &priority, QString low, int64_t &change)
{
    emitCoinControlUpdate(quantity, amount, fee, afterfee, bytes, priority, low, change);
}

QVariantMap CybCSecBridge::listAnonOutputs()
{
...
void CybCSecBridge::populateTransactionTable()
{
...
void CybCSecBridge::updateTransactions(QModelIndex topLeft, QModelIndex bottomRight)
{
...
void CybCSecBridge::insertTransactions(const QModelIndex & parent, int start, int end)
{
    // New Transactions...
    transactionModel->populateRows(start, end);
}

QString CybCSecBridge::transactionDetails(QString txid)
{
...
// Addresses
void CybCSecBridge::populateAddressTable()
{
...
void CybCSecBridge::updateAddresses(QModelIndex topLeft, QModelIndex bottomRight)
{
    addressModel->poplateRows(topLeft.row(), bottomRight.row());
}

void CybCSecBridge::insertAddresses(const QModelIndex & parent, int start, int end)
{
...
QString CybCSecBridge::newAddress(QString addressLabel, int addressType, QString address, bool send)
{
...
QString CybCSecBridge::lastAddressError()
{
    QString sError;
...
QString CybCSecBridge::getAddressLabel(QString address)
{
    return addressModel->atm->labelForAddress(address);
}

void CybCSecBridge::updateAddressLabel(QString address, QString label)
{
...
bool CybCSecBridge::validateAddress(QString address)
{
    return window->walletModel->validateAddress(address);
}

bool CybCSecBridge::deleteAddress(QString address)
{
    return addressModel->atm->removeRow(addressModel->atm->lookupAddress(address));
}

// Messages
void CybCSecBridge::appendMessages(QString messages, bool reset)
{
    emitMessages("[" + messages + "]", reset);
}

void CybCSecBridge::appendMessage(int row)
{
...
void CybCSecBridge::populateMessageTable()
{
...
void CybCSecBridge::insertMessages(const QModelIndex & parent, int start, int end)
{
    while(start <= end)
    {
        appendMessage(start++);
...
bool CybCSecBridge::deleteMessage(QString key)
{
    return window->messageModel->removeRow(thMessage->mtm->lookupMessage(key));
}

bool CybCSecBridge::markMessageAsRead(QString key)
{
    return window->messageModel->markMessageAsRead(key);
}

QString CybCSecBridge::getPubKey(QString address)
{
    return addressModel->atm->pubkeyForAddress(address);;
}

bool CybCSecBridge::setPubKey(QString address, QString pubkey)
{
...
bool CybCSecBridge::sendMessage(const QString &address, const QString &message, const QString &from)
{
    WalletModel::UnlockContext ctx(window->walletModel->requestUnlock());
...
QString CybCSecBridge::createGroupChat(QString label)
{
...
QString CybCSecBridge::joinGroupChat(QString privkey, QString label)
{
    /*
...
QVariantList CybCSecBridge::inviteGroupChat(QString qsaddress, QVariantList invites, QString from)
{
...
void CybCSecBridge::connectSignals()
{
...
QString CybCSecBridge::translateHtmlString(QString string)
{
    int i = 0;
...
QVariantMap CybCSecBridge::userAction(QVariantMap action)
{
...
// Blocks
QVariantMap CybCSecBridge::listLatestBlocks()
{
...
QVariantMap CybCSecBridge::findBlock(QString searchID)
{
...
QVariantMap CybCSecBridge::blockDetails(QString blkHash)
{
...
QVariantMap CybCSecBridge::listTransactionsForBlock(QString blkHash)
{
...
QVariantMap CybCSecBridge::txnDetails(QString blkHash, QString txnHash)
{
...
QVariantMap CybCSecBridge::signMessage(QString address, QString message)
{
    QVariantMap result;
...
QVariantMap CybCSecBridge::verifyMessage(QString address, QString message, QString signature)
{
    QVariantMap result;
...
QVariantMap CybCSecBridge::getNewMnemonic(QString password, QString language)
{
    QVariantMap result;
...
QVariantMap CybCSecBridge::importFromMnemonic(QString inMnemonic, QString inPassword, QString inLabel, bool fBip44, int64_t nCreateTime)
{
...
QVariantMap CybCSecBridge::extKeyAccList() {
    QVariantMap result;
...
QVariantMap CybCSecBridge::extKeyList() {
    QVariantMap result;
...
QVariantMap CybCSecBridge::extKeyImport(QString inKey, QString inLabel, bool fBip44, int64_t nCreateTime)
{
    QVariantMap result;
    ...
    if (eKey58.Set58(sInKey.c_str()) == 0)
    {
        ...
        {
            result.insert("error_msg", "Import failed - Key must begin with CybCSec prefix.");
            return result;
        }
...
QVariantMap CybCSecBridge::extKeySetDefault(QString extKeyID)
{
    QVariantMap result;
...
QVariantMap CybCSecBridge::extKeySetMaster(QString extKeyID)
{
    QVariantMap result;
    std::string sInKey = extKeyID.toStdString();
    if (extKeyID.length() == 0)
...
QVariantMap CybCSecBridge::extKeySetActive(QString extKeyID, QString isActive)
{
    QVariantMap result;
    std::string sInKey = extKeyID.toStdString();
