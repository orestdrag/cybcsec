...
QString TransactionRecord::getTypeLabel(const int &type)
{
    ...
    case RecvWithAddress:
        return CybCSecGUI::tr("Received with");
    case RecvFromOther:
        return CybCSecGUI::tr("Received from");
    case SendToAddress:
    case SendToOther:
        return CybCSecGUI::tr("Sent to");
    case SendToSelf:
        return CybCSecGUI::tr("Payment to yourself");
    case Generated:
        return CybCSecGUI::tr("Mined");
    case RecvCybCSec:
        return CybCSecGUI::tr("Received cybcsec");
    case SendCybCSec:
        return CybCSecGUI::tr("Sent cybcsec");
    case Other:
        return CybCSecGUI::tr("Other");
    default:
        return "";
    }
}
...
QString TransactionRecord::getTypeShort(const int &type)
{
    ...
    case TransactionRecord::RecvCybCSec:
        return "input";
    case TransactionRecord::SendToAddress:
    case TransactionRecord::SendToOther:
    case TransactionRecord::SendCybCSec:
        return "output";
    default:
        return "inout";
    }
...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet *wallet, const CWalletTx &wtx)
{
    ...
    int64_t nCredXCS, nCredCybCSec;
    wtx.GetCredit(nCredXCS, nCredCybCSec, true);
    int64_t nCredit = nCredXCS + nCredCybCSec;
    ...
    {
        if (nNet > 0 && nCredCybCSec > 0)
        {
            // -- credit
            TransactionRecord sub(hash, nTime, TransactionRecord::RecvCybCSec, "", "", nNet, 0);
        ...
        {
            // -- debit
            TransactionRecord sub(hash, nTime, TransactionRecord::SendCybCSec, "", "", nNet, 0);
        ...
        for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
        {
                ...
                && txout.IsAnonOutput())
            {
                ...
                if (wallet->HaveKey(ckidD))
                {
                    ...
                    sub.type = TransactionRecord::RecvCybCSec;
    ...
    } else
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, wtx.vin)
        {
                ...
                && txin.IsAnonInput())
            {
                ...
                {
                    fAllFromMe = false;
                    break; // display as send/recv cybcsec
                };
                continue;
            };
        ...
        if (fAllFromMe)
        {
            ...
            for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
            {
                    ...
                    && txout.IsAnonOutput())
                {
                    ...
                    sub.type = TransactionRecord::SendCybCSec;
                    sub.address = CBitcoinAddress(ckidD).ToString();
