...
int64_t CWallet::GetCybCSecDebit(const CTxIn& txin) const
{
    if (!txin.IsAnonInput())
        return 0;

    // - amount of owned cybcsec decreased
...
int64_t CWallet::GetCybCSecCredit(const CTxOut& txout) const
{
...
int64_t CWallet::GetCybCSecBalance() const
{
    int64_t nTotal = 0;

    {
        ...
        for (WalletTxMap::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            if (pcoin->IsTrusted() && pcoin->nVersion == ANON_TXN_VERSION)
                nTotal += pcoin->GetAvailableCybCSecCredit();
        };
    }
...
bool CWallet::SendSpecToAnon(CStealthAddress& sxAddress, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee)
{
    ...
    if (vNodes.empty())
    {
        sError = _("Error: CybCSec is not connected!");
        return false;
    };
...
bool CWallet::SendAnonToAnon(CStealthAddress& sxAddress, int64_t nValue, int nRingSize, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee)
{
    ...
    if (nValue + nTransactionFee > GetCybCSecBalance())
    {
        sError = "Insufficient cybcsec funds";
        return false;
    };
