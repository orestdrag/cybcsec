...
public:
    ...
    int64_t GetCybCSecBalance() const;
    ...
    int64_t GetCybCSecDebit(const CTxIn& txin) const;
    int64_t GetCybCSecCredit(const CTxOut& txout) const;
    ...
    int64_t GetCredit(const CTransaction& tx) const
    {
        ...
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txout.IsAnonOutput())
            {
                nCredit += GetCybCSecCredit(txout);
            } else
    ...
    bool GetCredit(const CTransaction& tx, int64_t& nXCS, int64_t& nCybCSec) const
    {
        nXCS = 0;
        nCybCSec = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txout.IsAnonOutput())
            {
                nCybCSec += GetCybCSecCredit(txout);
            } else
...
public:
    ...
    mutable bool fAvailableCybCSecCreditCached;
    mutable bool fCreditSplitCached;
    mutable int64_t nDebitCached;
    mutable int64_t nCreditCached;
    mutable int64_t nAvailableCreditCached;
    mutable int64_t nChangeCached;
    mutable int64_t nAvailableCybCSecCreditCached;
    
    mutable int64_t nCredXCSCached;
    mutable int64_t nCredCybCSecCached;
    ...
    void Init(const CWallet* pwalletIn)
    {
        ...
        fAvailableCybCSecCreditCached = false;
        
        nCredXCSCached = 0;
        nCredCybCSecCached = 0;
        ...
        nAvailableCybCSecCreditCached = 0;
        nChangeCached = 0;
        nOrderPos = -1;
    }
    ...
    bool GetCredit(int64_t& nCredXCS, int64_t& nCredCybCSec, bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
        {
            nCredXCS = nCredCybCSec = 0;
            return true;
        }
        ...
        if (!fUseCache || !fCreditSplitCached)
        {
            nCredXCSCached = 0;
            nCredCybCSecCached = 0;
            pwallet->GetCredit(*this, nCredXCSCached, nCredCybCSecCached);
            fCreditSplitCached = true;
        };
        
        nCredXCS = nCredXCSCached;
        nCredCybCSec = nCredCybCSecCached;
        return true;
    }
    ...
    int64_t GetAvailableCybCSecCredit(bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
            return 0;
        
        if (fUseCache && fAvailableCybCSecCreditCached)
            return nAvailableCybCSecCreditCached;
        ...
        for (unsigned int i = 0; i < vout.size(); i++)
        {
            if (!IsSpent(i))
            {
                ...
                if (!MoneyRange(nCredit))
                    throw std::runtime_error("CWalletTx::GetAvailableCybCSecCredit() : value out of range");
            };
        };
        
        nAvailableCybCSecCreditCached = nCredit;
        fAvailableCybCSecCreditCached = true;
        return nCredit;
    };
